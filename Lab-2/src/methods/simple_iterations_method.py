...
from Derivative.d import derivative
...
class SimpleIterationsMethod(Method):
    ...
    def solve(self) -> Result:
        ...
        max_derivative = max(abs(derivative(f, self.left)), abs(derivative(f, self.right)))
        ...
        print('phi\'(a) = ', abs(derivative(phi, self.left, dx=dx)))
        print('phi\'(b) = ', abs(derivative(phi, self.right, dx=dx)))
        for x in numpy.linspace(self.left, self.right, steps, endpoint=True):
            if abs(derivative(phi, x, dx=dx)) >= 1:
